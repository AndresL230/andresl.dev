...
export const projects: Project[] = [
  ...
  },
  {
    ...
    desc: 'Shared context store for a team working with coding agents. One Cloudflare Worker serving an HTTP API, an MCP endpoint, and a SPA, with an idempotent gate between the agents and anything published.',
    ...
    gallery: [
      ...
      { src: '/images/canopy_docs.png', caption: 'Canopy · doc tree with a staged proposal awaiting review' },
      { src: '/images/canopy_feed.png', caption: 'Canopy · session feed, each entry carrying its PR and commit artifacts' },
    ],
    tech: [
      'TypeScript',
      'Cloudflare Workers',
    ...
    role: 'solo · 283 of 284 commits',
    problem:
      "A four-person team shipping with coding agents kept losing the same context. Decisions lived in closed PR threads, architecture notes drifted from the code, and every new agent session started from zero, re-deriving what had already been settled. Letting agents write to a shared store fixes the amnesia and creates a worse problem: nothing an LLM produces is trustworthy enough to publish unreviewed.",
    solution:
      'Canopy is that shared store, running as one Cloudflare Worker. Agents read assembled context through an MCP endpoint and propose changes back through the same endpoint. Every proposed write lands staged; a human promotes, rejects, or routes it to triage from the web UI. Agents produce, humans confirm, enforced by the architecture rather than by convention.',
    deepDive: [
      'One Worker, three surfaces off one origin: a Hono JSON API, a stateless MCP endpoint at /mcp built on the MCP TypeScript SDK, and the SPA served through the ASSETS binding. D1 is the only datastore — no KV, no Durable Objects, no external search service. ~15,500 hand-written lines across worker, SPA, tests, and SQL.',
      'One write gate, 346 lines, that every agent write funnels through regardless of entry point. A replay ledger keyed on (session_id, item_index) drops a re-POSTed payload; SHA-256 content hashes drop no-op edits; an LCS line diff types the rest as edit or rewrite at a 0.5 changed-over-max threshold; low confidence on a new slug routes to a human triage queue instead of the doc tree.',
      'Agents can only ever stage. Every confirm verb — promote, ratify, reject, assign, discard — is a session-cookie HTTP route and is deliberately never registered as an MCP tool. The 11 MCP tools are built per request against an authenticated principal, so the admin plan-write tool is absent from tools/list entirely for everyone else rather than failing at call time.',
      'Retrieval is four FTS5 virtual tables with trigger-maintained indexes and bm25 weighted 5x toward titles, with query tokens sanitized and phrase-quoted so FTS5 operators cannot be injected. Every result carries an authority flag (live, staged_pending, unpromoted, draft) so an agent can tell settled context from an unreviewed proposal.',
      'A 439-test suite across 54 files runs against a real Miniflare D1, proving replay-safety at three layers: the reconciler unit, the HTTP route, and the MCP tool. Ships as a Claude Code plugin — six skills plus auto-wired MCP config, installable from a marketplace manifest in the repo.',
    ],
  },
  {
    slug: 'calyx',
